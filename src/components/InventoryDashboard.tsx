...
/**
 * Display a UOS rate without forcing two decimals — unit_price is DECIMAL(18,6) and a
 * correction may legitimately need all six, so trailing zeros are trimmed rather than padded.
 */
const formatUosRate = (value: string | number | null | undefined): string => {
...
export function InventoryDashboard({ role }: InventoryDashboardProps) {
  ...
  // UOS row being corrected (null = modal closed). Only UNTOUCHED rows can open this;
  // the backend re-checks that under FOR UPDATE, so this is a convenience gate only.
  const [editingUos, setEditingUos] = useState<any>(null);
  const [uosEditQty, setUosEditQty] = useState('');
  const [uosEditRate, setUosEditRate] = useState('');
  const [uosEditReason, setUosEditReason] = useState('');
  const [isSavingUos, setIsSavingUos] = useState(false);
  ...
  const openUosEdit = (uos: any) => {
    setEditingUos(uos);
    // Seed from the stored values without reformatting, so the six-decimal rate survives.
    setUosEditQty(String(uos.quantity_received));
    setUosEditRate(String(uos.unit_price));
    setUosEditReason('');
  };

  /**
   * Save a correction to an untouched UOS layer.
   * Sends quantity, rate and reason only — the backend derives every value column and
   * re-verifies that the layer is still untouched inside its transaction.
   */
  const handleSaveUosEdit = async () => {
    if (!editingUos) return;
    const qty = parseFloat(uosEditQty);
    const rate = parseFloat(uosEditRate);
    if (isNaN(qty) || qty <= 0) { alert('Quantity must be greater than zero.'); return; }
    if (isNaN(rate) || rate <= 0) { alert('Rate must be greater than zero.'); return; }
    if (uosEditReason.trim().length < 5) { alert('An edit reason of at least 5 characters is required.'); return; }

    setIsSavingUos(true);
    try {
      const res = await fetch(`${API_CONFIG.BASE_URL}/inventory/opening-stock/${editingUos.id}`, {
        method: 'PUT',
        headers: createAuthHeaders(true),
        body: JSON.stringify({ quantity: qty, unit_cost: rate, reason: uosEditReason.trim() })
      });
      const body = await res.json();
      if (res.ok) {
        setEditingUos(null);
        fetchData();
        alert(body.changed === false ? 'No changes to apply.' : 'Opening stock updated successfully');
      } else {
        alert(body.error || 'Failed to update opening stock');
      }
    } catch (e) {
      alert('Failed to update opening stock');
    } finally {
      setIsSavingUos(false);
    }
  };
    ...
    <>
      ...
      {activeSubTab === 'inventory' ? (
        ...
        <div className="space-y-4">
          <FilterBar
            searchPlaceholder="Search item, UOS number, category..."
            filterType="none"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50 text-gray-500 text-xs uppercase tracking-wider">
                  ...
                  <th className="px-6 py-4 font-medium">UOS Number</th>
                  <th className="px-6 py-4 font-medium">Item Details</th>
                  <th className="px-6 py-4 font-medium text-right">Qty Added</th>
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {additions.length === 0 ? (
                  <tr><td colSpan={10} className="px-6 py-12 text-center text-gray-500">No opening stock (UOS) records found.</td></tr>
                ) : additions.map((addition) => {
                  const editable = addition.status === 'UNTOUCHED';
                  return (
                  <tr key={addition.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap text-gray-600">{new Date(addition.received_date).toLocaleDateString()}</td>
                    <td className="px-6 py-4">
                      <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-black uppercase bg-blue-50 text-blue-600 border border-blue-100">
                        {addition.batch_number}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="font-bold text-gray-900">{addition.item_name}</div>
                      <div className="text-[10px] text-gray-400">{addition.category || 'Uncategorised'}</div>
                    </td>
                    <td className="px-6 py-4 text-right font-medium text-emerald-700">+{addition.quantity_received} {addition.unit || ''}</td>
                    <td className="px-6 py-4 text-right font-medium text-gray-700">{addition.quantity_remaining}</td>
                    <td className="px-6 py-4 text-right font-medium text-gray-900 tabular-nums">₹{formatUosRate(addition.unit_price)}</td>
                    <td className="px-6 py-4 text-right font-black text-slate-900">₹{parseFloat(addition.total_value_received || 0).toLocaleString()}</td>
                    <td className="px-6 py-4 text-center">
                      <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-black uppercase border ${
                        addition.status === 'UNTOUCHED' ? 'bg-emerald-50 text-emerald-600 border-emerald-100'
                        : addition.status === 'FULLY CONSUMED' ? 'bg-red-50 text-red-600 border-red-100'
                        : addition.status === 'VOIDED' ? 'bg-gray-100 text-gray-500 border-gray-200'
                    ...
                    <td className="px-6 py-4 text-center">
                      {canModifyInventory && editable ? (
                        <button
                          type="button"
                          onClick={() => openUosEdit(addition)}
                          className="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-black uppercase text-blue-600 bg-blue-50 border border-blue-200 hover:bg-blue-100 transition-colors"
                        >
                        ...
                        <span
                          ...
                          title={editable ? 'You do not have permission to edit inventory' : 'Only untouched opening stock can be edited'}
                        >
                          —
                        </span>
                      )}
      ...
      {/* UOS Correction Modal — only quantity and rate are editable; every other field is fixed */}
      {editingUos && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-blue-50">
              ...
              <button onClick={() => setEditingUos(null)} className="text-gray-400 hover:text-gray-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="bg-gray-50 rounded-lg border border-gray-200 divide-y divide-gray-100 text-sm">
                <div className="px-4 py-2 flex justify-between">
                  <span className="text-gray-500">UOS Number</span>
                  <span className="font-bold text-gray-900">{editingUos.batch_number}</span>
                </div>
                <div className="px-4 py-2 flex justify-between">
                  <span className="text-gray-500">Item</span>
                  <span className="font-bold text-gray-900 text-right">{editingUos.item_name}</span>
                </div>
                <div className="px-4 py-2 flex justify-between">
                  <span className="text-gray-500">Current Quantity</span>
                  <span className="font-bold text-gray-900">{editingUos.quantity_received} {editingUos.unit || ''}</span>
                </div>
                <div className="px-4 py-2 flex justify-between">
                  <span className="text-gray-500">Current Rate</span>
                  <span className="font-bold text-gray-900 tabular-nums">₹{formatUosRate(editingUos.unit_price)}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">New Quantity</label>
                  <input
                    type="number" step="any" min="0"
                    value={uosEditQty}
                    onChange={(e) => setUosEditQty(e.target.value)}
                  ...
                  />
                  <p className="mt-1 text-[10px] text-gray-400">Stored to 2 decimal places</p>
                </div>
                <div>
                  ...
                  <input
                    type="number" step="any" min="0"
                    value={uosEditRate}
                    onChange={(e) => setUosEditRate(e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20"
                  />
                  <p className="mt-1 text-[10px] text-gray-400">Up to 6 decimal places</p>
                </div>
              </div>

              <div className="bg-blue-50 border border-blue-200 rounded-lg px-4 py-2 text-xs font-bold text-blue-700 flex justify-between">
                <span>New Total Value</span>
                <span>
                  ₹{(() => {
                    const q = parseFloat(uosEditQty), r = parseFloat(uosEditRate);
                    return Number.isFinite(q) && Number.isFinite(r)
                      ? (q * r).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 2 })
                      : '—';
                  })()}
                </span>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Edit Reason (Governance Audit)</label>
                <textarea
                  rows={3}
                  value={uosEditReason}
                  onChange={(e) => setUosEditReason(e.target.value)}
                  placeholder="Mandatory explanation for this correction (min 5 characters)..."
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 resize-none"
                />
              </div>
            </div>
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end space-x-3">
              ...
              <button
                type="button"
                disabled={isSavingUos || uosEditReason.trim().length < 5}
                onClick={handleSaveUosEdit}
                className="px-5 py-2 bg-blue-600 text-white text-sm font-black rounded-lg hover:bg-blue-700 shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSavingUos ? 'Saving...' : 'Save Changes'}
              </button>
            </div>
          </div>
        </div>
